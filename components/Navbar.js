...
const Navbar = ({toggle, isOpen}) => {
    ...
    return (
        <div class=' flex flex-row flex-nowrap justify-start sm:justify-around h-8 w-full py-8 items-center mt-10 sm:mb-24 select-none'>
            <div className="w-1/8 flex hover:scale-105 duration-300 ml-7 mb-7 cursor-pointer sm:hidden select-none" onClick={toggle}>
                <svg class={`w-8 h-8 ${isOpen ? 'hidden' : ''}`}  fill="currentColor" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
                </svg>
                <svg class={`w-8 h-8 ${!isOpen ? 'hidden' : ''}`} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" onClick={toggle}>
                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                </svg> 
            </div>
            <Link href="/"> 
                <a class="hover:text-[gray] hidden lg:block font-medium transiton ease-linear duration-300 text-sm lg:text-base w-1/5 text-center" >Nabil Chowdhury</a>
            </Link>
            <nav>
            ...
            <div class='font-medium hidden lg:block  hover:text-[gray] text-sm lg:text-base transiton ease-linear duration-300 px-2 w-1/5 text-center'>
